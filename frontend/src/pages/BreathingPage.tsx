...
export default function BreathingPage() {
  ...
  // Dynamic scale for breathing circle
        ...
        <div className="py-4 animate-fade-in">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-calm-100 mb-6">
            ...
            <p className="text-calm-600 text-sm leading-relaxed mb-4">
              {slug.includes('478') ? (
                <>4-7-8 呼吸法被称为"神经系统的天然镇静剂"。通过特定的节奏（吸气4秒、屏息7秒、呼气8秒），激活副交感神经，帮助身体从紧张模式切换到放松状态。</>
              ) : slug.includes('box') ? (
      ...
      {state === 'completed' && (
        <div className="text-center py-12 animate-spring-in">
          ...
          <div className="flex justify-center gap-3">
            <button onClick={() => { setShowIntro(true); stop(); }} className="rounded-full border border-calm-300 text-calm-600 px-6 py-3">再来一次</button>
            <button onClick={() => navigate('/')} className="rounded-full bg-calm-500 text-white px-8 py-3 font-semibold">返回首页</button>
          </div>
        </div>
      )}

      <AICoach guideType="breathing" currentPhase={phaseName} triggerKey={currentRound} enabled={state === 'running'} />
    </Layout>
  );
}
